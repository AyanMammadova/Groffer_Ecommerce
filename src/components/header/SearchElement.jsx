...
function SearchElement() {
    ...
    function handleSearch() {
        const trimmedInput = untrimmed.replace(/ /g, "")
        ...
        <>
            <div className='hidden md:block'>
                <div className='bg-[#F2F2F2]  relative  flex items-center rounded-sm'>
                    <input
                        onChange={(e) => { setUntrimmed(e.target.value),handleSearch() }}
                        type="text"
                        value={untrimmed}
                    ...
                    <div className={` ${searchedValue.length > 0 ? 'absolute' : 'hidden'} z-50 max-h-[300px] overflow-y-scroll   top-[60px]   w-[100%] bg-white  shadow-lg mt-[10px] border-[1px] border-gray-100`}>
                        {
                            ...
                            }) : searchedProducts && searchedProducts?.length == 0 ? <p className='max-w-[100%]  break-words'>No product found for {searchedValue}'</p> : ''
                        }
                    </div>
                </div>
