...
function ProductCard({ id }) {
    const { favoriteData,handleWishlist } = useContext(DATA)
            ...
            <Link
                ...
                className='group-hover:block hidden'>
                    
                    {
                        isFav 
                        ? <IoIosHeart
                        onClick={()=>{handleWishlist(singlePro.id, isFav)}}
                         className={` absolute text-[1.3em] text-red-700 cursor-pointer top-[50px] right-[10px]`} />
                        : <IoIosHeartEmpty
                        onClick={()=>{handleWishlist(singlePro.id, isFav)}}
                         className={` absolute text-[1.3em] text-gray-700 cursor-pointer hover:text-black top-[50px] right-[10px]`} />
                    }
                    ...
                    <FaMagnifyingGlass
                        ...
                        className='absolute  top-[70px] right-[10px] text-gray-700 hover:text-black' />
                </div>
            </Link>
        </>
    )
