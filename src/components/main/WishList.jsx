...
function WishList() {
    ...
    const { basketData, removeFromBasket, addToBasket, loading } = useContext(BASKET)
                        ...
                        ? loadingWishlist ? (
                            ...
                            favoriteData?.length > 0
                                ? favoriteData?.map((item, i) => (
                                    <Link to={`/details/${item?.id}`} key={i} className='w-[96%] sm:w-[45%] md:w-[90%] mb-[30px] items-center  justify-between px-[10px] md:px-[40px] flex-wrap flex mx-auto shadow-[0_0px_20px_rgba(0,0,0,0.1),0_1px_3px_rgba(0,0,0,0.08)] py-[20px] mt-[20px]'>
                                        <div className='flex gap-[20px] w-[100%] md:w-auto justify-between relative md:items-center'>
                                            <div>
                                                <IoCloseSharp
                                                    onClick={(e) => { toggleWishlist(item.id), e.preventDefault() }}
                                        ...
                                        {
                                            item?.discount > 0
                                                ? <p className='text-[#216D5A] px-[10px] font-bold  text-[1em] '>
                                                    <del className='text-gray-900 px-[5px]'>€{item.price}</del> €{item.discountedPrice}
                                                </p>
                                                : <p className='text-[#216D5A] px-[10px] font-bold text-[1em] ]'>
